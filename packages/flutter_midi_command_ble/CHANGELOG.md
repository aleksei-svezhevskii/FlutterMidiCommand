## 1.1.0
 ...
 - FIX: absorb the failure from `stopScan` instead of dropping the future. Android rejects a stop once the adapter has been switched off — an ordinary thing for a user to do mid-scan — and both call sites are `void`, so the rejection had no listener and surfaced as an unhandled error in the host app's crash reporting.
